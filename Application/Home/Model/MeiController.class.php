...
class MeiController extends Controller {
	...
	public function reservation_home_update(){
		...
		$data['type'] = '0';
		$data['xiaofei'] = $mei[0]['price'] - I('post.coins');
		...
		if($mei[0]['jifen'] >= I('post.coins')){
			if($login[0]['yue'] >= I('post.coins')){
				...
				if (I('post.name')) {
					$m = M('dingdan');
					if ($m->data($data)->add()) {
						$this->senmes($mei_id,$time);
						echo  "<script>alert('预约成功，请耐心等待！');window.location.href='http://serv2.matesofts.com/weimei/index.php/Home/Mei/mei_index'</script>";
					} else {
						echo  "<script>alert('预约失败，请重新预约！');window.location.href='http://serv2.matesofts.com/weimei/index.php/Home/Mei/mei_index'</script>";
					}
				}

			}else{
				echo  "<script>alert('维美币不足！');window.location.href='http://serv2.matesofts.com/weimei/index.php/Home/Mei/mei_index'</script>";
			}
		}else{
			echo  "<script>alert('输入的维美币过多！');window.location.href='http://serv2.matesofts.com/weimei/index.php/Home/Mei/mei_index'</script>";
		}
...
//地址
	...
	public function address_add_update(){
		...
		$data['wxid'] = $openid;
		$m = M('address');
		if($m->where($map)->add($data)){
			echo  "<script>alert('添加成功！');window.location.href='http://serv2.matesofts.com/weimei/index.php/Home/Mei/mei_index'</script>";
		}else{
			echo  "<script>alert('添加失败！');window.location.href='http://serv2.matesofts.com/weimei/index.php/Home/Mei/mei_index'</script>";
		}
	}
	...
	public function address_modify_update(){
		...
		$data['qu'] = I('post.qu');
		$data['mobile'] = I('post.mobile');
		$map['wxid'] = I('post.openid');
		$m = M('address');
		if($m->where($map)->save($data)){
			echo  "<script>alert('修改成功！');window.location.href='http://serv2.matesofts.com/weimei/index.php/Home/Mei/mei_index'</script>";
		}else{
			echo  "<script>alert('修改失败！');window.location.href='http://serv2.matesofts.com/weimei/index.php/Home/Mei/mei_index'</script>";
		}
...
//我的会员
	...
	public function activity_update(){
		...
		if($long == 0){
			...
			$m->data($data)->add();
			echo  "<script>alert('您已成功参加了此次活动！');window.location.href='http://serv2.matesofts.com/weimei/index.php/Home/Mei/mei_index'</script>";
		}else{
			echo  "<script>alert('别贪心，下次再来吧！');window.location.href='http://serv2.matesofts.com/weimei/index.php/Home/Mei/mei_index'</script>";
		}
	...
	//发送预约成功通知
	public function senmes($mei_id,$time)
	{
            ...
            $data=array(
                ...
                'remark'=>array('value'=>urlencode('请尽快处理'),'color'=>'#666'),
            );
            //dump($data); 
            $m=M('admininfo'); 
            $list=  $m->select();
