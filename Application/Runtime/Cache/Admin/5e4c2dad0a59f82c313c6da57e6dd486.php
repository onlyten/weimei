...
<body class=" theme-blue">
	...
	<div class="main-content">
		<form enctype="multipart/form-data" action="<?php echo U('server_delete_update');?>" method="post" >
			<div class="btn-toolbar list-toolbar col-md-12" style="margin-top: -20px;">　　　　　　　　　　　　　　　　　　　
			    <div class='col-md-2'> <a href="<?php echo U('server_add');?>" class="btn btn-primary"><i class="fa fa-plus"></i> 添加服务类型</a></div>
			    <div class='col-md-2'><button type="submit" class="btn btn-danger" name="del" ><i class="glyphicon glyphicon-trash"></i> 删除服务类型</button></div>
			</div>
			...
			<div class="row">
				<div class="col-md-12">
					<table class="table table-striped">
						...
						<tbody>
							<?php if(is_array($server)): $i = 0; $__LIST__ = $server;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
								<td> <input type="checkbox" name="delete_id[]" value="<?php echo ($vo['id']); ?>">&nbsp;&nbsp;<?php echo ($vo['id']); ?></td>
								<td><?php echo ($vo['server_name']); ?></td>
								<td>
									<?php if($vo['server_type'] == 1 ): ?>美容项目
									    <?php else: ?> 美丽管家<?php endif; ?>
								</td>
								<td><img class="media-object thumb-sm" src="http://serv2.matesofts.com/weimei/Public/upload/img/<?php echo ($vo['server_img']); ?>"></td>
								<td>
									<a href="<?php echo U('server_modify',array('server_id'=>$vo['id']));?>">编辑</a>
								</td>
	...
	<script type="text/javascript">
		$('.next_page').on('click', function() {
			...
			if (<?php echo ($page_now); ?> == <?php echo ($server_pagenum); ?>) {
				...
				location.href = "<?php echo U('server_list',array('page_now'=>$page_now+1));?>";
			};
		});
		...
		$('.jump_button').on('click', function() {
			...
			if ( parseInt($('.jump_page').val()) < 1 || parseInt($('.jump_page').val()) > <?php echo ($server_pagenum); ?> ) {
				...
				location.href = "<?php echo U('server_list');?>/page_now/"+$('.jump_page').val();
			};
		});
	</script>
