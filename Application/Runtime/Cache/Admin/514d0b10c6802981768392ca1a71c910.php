...
<body class=" theme-blue">
	...
	<div class="header">
        <h1 class="page-title">美丽管家管理</h1>
        <ul class="breadcrumb">
            <li>美丽管家管理</li> 
            <li class="active">美丽管家管理</li>          
        </ul>
    </div>
	...
	<div class="main-content">
		<form enctype="multipart/form-data" action="<?php echo U('housekeeper_delete_update');?>" method="post" >
			<div class="btn-toolbar list-toolbar col-md-12" style="margin-top: -20px;">　　　　　　　　　　　　　　　　　　　
			    <div class='col-md-2'> <a href="<?php echo U('housekeeper_add');?>" class="btn btn-primary"><i class="fa fa-plus"></i> 添加美丽管家</a></div>
			    <div class='col-md-2'><button type="submit" class="btn btn-danger" name="del" ><i class="glyphicon glyphicon-trash"></i> 删除美丽管家</button></div>
			</div>
			...
			<div class="row">
				<div class="col-md-12">
					<table class="table table-striped">
						...
						<tbody>
							<?php if(is_array($housekeeper)): $i = 0; $__LIST__ = $housekeeper;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
								<td> <input type="checkbox" name="delete_id[]" value="<?php echo ($vo['id']); ?>">&nbsp;&nbsp;<?php echo ($vo['id']); ?></td>
								<td><?php echo ($vo['job_num']); ?></td>
								<td><?php echo ($vo['name']); ?></td>
								<td><?php echo ($vo['xueli']); ?></td>
								<td><img class="media-object thumb-sm" src="http://serv2.matesofts.com/weimei/Public/upload/img/<?php echo ($vo['pic_tou']); ?>"></td>
								<td><?php echo ($vo['cishu']); ?></td>	
								<td><?php echo ($vo['dashang']); ?></td>
															
								<td>
									<a href="<?php echo U('housekeeper_modify',array('housekeeper_id'=>$vo['id']));?>">编辑</a>
								</td>
	...
	<script type="text/javascript">
		$('.next_page').on('click', function() {
			...
			if (<?php echo ($page_now); ?> == <?php echo ($housekeeper_pagenum); ?>) {
				alert('当前页为最后一页');
			}else{
				location.href = "<?php echo U('housekeeper_list',array('page_now'=>$page_now+1));?>";
			};
		});
		...
		$('.jump_button').on('click', function() {
			order_condition = $('#order_condition').val();
			page_condition = $('#page_condition').val();
			if ( parseInt($('.jump_page').val()) < 1 || parseInt($('.jump_page').val()) > <?php echo ($housekeeper_pagenum); ?> ) {
				alert('超过页数范围');
			}else{
				location.href = "<?php echo U('housekeeper_list');?>/page_now/"+$('.jump_page').val();
			};
		});
	</script>
